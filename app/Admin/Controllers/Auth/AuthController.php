...
class AuthController extends Controller
{
    ...
    public static function generateCaptcha(int $length = 4, int $width = 150, int $height = 50, int $size = 24, string $font = ''): object
    {
...
BASE64;
        ...
        foreach($code as $index => $char) {
            ...
            $offsetX = (int)($index * $textWidth + ($index + 1) * $x);
            $offsetY = (int)$y;
            imagettftext($image, (float)$size, (float)$angle, $offsetX, $offsetY, $color, $font, $char);
        }
        ...
        return (object)['code' => implode('', $code), 'data' => $data];
    }
    ...
    public function login(Request $request)
    {
        ...
        if(config('admin.auth.verification')) {

            $captcha = $request->session()->pull('captcha', '');

            if(strcasecmp($request->input('captcha'), $captcha) !== 0) {
                $captcha = self::generateCaptcha();
