...
class UserLoginController extends TimeTabController
{
    public function index(Content $content)
    {
        ...
        if ($current == "day")
        {
            $step = 3600;
            $format = "'H'";
            $data = DB::select("SELECT COUNT(DISTINCT `role_id`) AS `number`, CONCAT(\"'\", DATE_FORMAT(FROM_UNIXTIME(`login_time`), '%H'), \"'\") AS `date` FROM `{$database}`.`login_log` WHERE `login_time` BETWEEN ? AND ? GROUP BY `date`", [$before, $now]);
        }
        else
        {
            $step = 86400;
            $format = "'m-d'";
            $data = DB::select("SELECT COUNT(DISTINCT `role_id`) AS `number`, CONCAT(\"'\", DATE_FORMAT(FROM_UNIXTIME(`time`), '%m-%d'), \"'\") AS `date` FROM `{$database}`.`login_log` WHERE `time` BETWEEN ? AND ? GROUP BY `date`", [$before, $now]);
        }
        // chart data
        if (empty($data))
        {
