...
class UserRegisterController extends Controller
{
    public function index(Content $content)
    {
        ...
        // nav
        $list = implode("", array_map(function($time, $text) { if($time == request()->input("time", 1)) return "<li role='presentation' class='active' ><a>" . trans("admin." . $text) . "</a></li>"; else return "<li role='presentation' ><a href='user-register?time={$time}'>" . trans("admin." . $text) . "</a></li>"; }, array(1, 7, 30, SwitchServerController::getCurrentServerOpenDays()), array("day", "week", "month", "all")));        // draw
        return $content->body("
            ...
            $(function () {
                echarts.init(document.getElementById('chart'), 'shine').setOption({
                    ...
                    series: [{type: 'line', smooth: true, data: [{$register}]}]
                });
            });
            </script>
        ");
