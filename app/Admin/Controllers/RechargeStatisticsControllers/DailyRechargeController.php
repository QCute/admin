...
class DailyRechargeController extends TimeTabController
{
    public function index(Content $content)
    {
        ...
        $data = DB::select("SELECT sum(`money`) AS `value`, CONCAT(\"'\", DATE_FORMAT(FROM_UNIXTIME(`time`), '%m-%d'), \"'\") AS `name` FROM `{$database}`.`recharge` WHERE `time` BETWEEN ? AND ? GROUP BY `name`", [$before, $now]);
        // $data = array_map(function($object){ $object->label = ["normal" => ["show" => true, "position" => "top", "formatter" => "{c}"]]; return $object; }, $data);
        // chart data
        if (empty($data))
        {
            $category = [];
            for ($start = $before; $start <= $now; $start += 86400) 
            {
                ...
                // array_push($data, ["value" => 0, "label" => ["normal" => ["show" => true, "position" => "top", "formatter" => "{c}"]]]);
            }
        ...
        return $content->title('')->body("
            ...
            $(function () {
                echarts.init(document.getElementById('chart'), 'shine').setOption({
                    ...
                    yAxis: { type: 'value', splitLine: { show: false }, axisTick: { show: false }, axisLabel: { textStyle: { color: '#556677' }, formatter: function(value) { return format(value); } }, axisLine: { lineStyle: { color: '#DCE2E8' } } },
                    series: [{ type: 'bar', itemStyle: { normal: { color: '#37a2da' } }, label: { normal: { show: true, position: 'top', formatter: function(object) { return format(object.value); } } }, data: " . json_encode($data) . " }]
                });
            });
            </script>
        ");
