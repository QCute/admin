...
class ChargeRateController extends ChartController
{
        ...
        for($date = $before; $date < $now; $date += 86400) {
            ...
            for($day = 0; $day < 30; $day++) {
                ...
                $sub = SwitchServerController::getDB()
                    ...
                    ->select([
                        DB::raw("CONCAT(IFNULL(COUNT(DISTINCT charge.`role_id`), 0), '/', COUNT(DISTINCT login_log.`role_id`), '(', FORMAT(IFNULL(IFNULL(COUNT(DISTINCT charge.`role_id`), 0) * 100 / COUNT(DISTINCT login_log.`role_id`), 0), 2), '%', ')') AS `day_$day`"),
                    ]);
                // row sub
