...
class WeChatService
{
     ...
     * @return array
     */
    public function request(string $method = 'GET', string $path, array $headers = [], array $data = [], int $timeout = 60): array
    {
        $result = [];

        if($method == 'GET') {
            $query = http_build_query($data);
            $result = Http::withHeaders($headers)->timeout($timeout)->get(Self::$url, $query)->json();
        }
        
        if($method == 'POST') {
            $result = Http::withHeaders($headers)->timeout($timeout)->post(Self::$url, $data)->json();
        }

        $code = $result['errcode'] ?? $result['err_no'] ?? 0;
        if($code > 0) {
            $err = $result['errcode'] ?? 0;
            $msg = $result['errmsg'] ?? '';
    ...
    public function getAccessToken(): array
    {
        ...
        $result = $this->request('GET', 'cgi-bin/token', [], $data);
        $result['time'] = time() - 60;
    ...
    public function login(string $code): array
    {
        ...
        return $this->request('GET', 'sns/jscode2session',  [], $data);
    }
}
